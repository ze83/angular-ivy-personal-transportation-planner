...
import {
  selectTransports,
  selectFavoriteTransports,
  areTransportsLoaded,
  loading
} from "../shared/transport.selectors";
...
export class TransportsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.favoritesConnections$ = this.store.pipe(
      select(selectFavoriteTransports)
    );
  ...
  loadConnections() {
    ...
    this.store.dispatch(
      TransportActions.loadTransportParam({
        transport: {
          from: this.searchform.controls.from.value,
          to: this.searchform.controls.to.value,
          date: this.transformDateFormat(
            this.searchform.controls.datetime.value
          ),
